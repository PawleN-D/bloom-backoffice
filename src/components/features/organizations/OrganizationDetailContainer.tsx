...
type OrganizationDetailContainerProps = {
  organizationId: string;
  initialTab?: string | null;
};

function normalizeTab(value?: string | null): Tab {
  const next = tabs.find((tab) => tab.toLowerCase() === String(value ?? "").toLowerCase());
  return next ?? "Overview";
}

function getHealthBreakdown(organization: OrganizationSummary, users: OrganizationUser[]) {
  const userUsagePct = organization.usersLimit
    ? Math.min(100, Math.round((organization.usersUsed / organization.usersLimit) * 100))
    : 0;
  const clientUsagePct = organization.clientsLimit
    ? Math.min(100, Math.round((organization.clientsUsed / organization.clientsLimit) * 100))
    : 0;
  const usage = 100 - Math.max(userUsagePct, clientUsagePct) / 2;

  const lastActivityDays = organization.lastActivityAt
    ? Math.floor((Date.now() - new Date(organization.lastActivityAt).getTime()) / (24 * 60 * 60 * 1000))
    : 30;
  const activity = Math.max(10, 100 - lastActivityDays * 3);

  const stuckUsers = users.filter((user) => {
  ...
  const communications = Math.max(10, 100 - stuckUsers * 20);

  return {
    compliance: Math.max(10, Math.min(100, organization.healthScore + 8)),
...
export function OrganizationDetailContainer({
  organizationId,
  initialTab,
}: OrganizationDetailContainerProps) {
  ...
  const isOrgReadOnly = organization?.status === "SUSPENDED";
  ...
  const handleInviteUser = async () => {
    if (!canManageUsers || isOrgReadOnly) return;
  ...
  const handleSendResetLink = async (userId: string, email: string) => {
    ...
    setUserActionError(null);
    setUserActionNotice(null);
    setResetActionId(userId);
    try {
      await sendUserResetLink(userId);
      setUserActionNotice(`Password reset link sent to ${email}`);
    } catch (err) {
      const details = getApiErrorDetails(err, "Unable to send reset link.");
      setUserActionError(details.message);
      setUserActionDebug({ status: details.status, data: details.data });
    } finally {
      setResetActionId(null);
  ...
  const isOlderThanSevenDays = (value: string | null | undefined) => {
  ...
  const isNeverLoggedIn = (userRow: OrganizationUser) =>
    userRow.lastLoginAt === null && isOlderThanSevenDays(userRow.createdAt);

  const isSetupIncomplete = (userRow: OrganizationUser) =>
    Boolean(userRow.mustResetPw) && isOlderThanSevenDays(userRow.createdAt);
  ...
  const clientsPercent = organization.clientsLimit
    ? Math.min(100, Math.round((organization.clientsUsed / organization.clientsLimit) * 100))
    : 0;
  const healthBreakdown = getHealthBreakdown(organization, users);
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <OrganizationActions
            ...
            variant="button"
            permissions={{
              canEdit: canManageOrg && !isOrgReadOnly,
              canSuspend: canManageOrg && !isOrgReadOnly,
              canDelete: canManageOrg && !isOrgReadOnly,
              canViewSubscription,
              canViewActivity,
            }}
          />
      ...
      {isOrgReadOnly ? (
        <Card className="border-warning-500/30 bg-warning-500/10">
          <p className="text-sm text-warning-500">
            This organisation is suspended. Data remains visible in read-only mode for compliance and audit review.
          </p>
        </Card>
      ) : null}
      ...
      <div className="flex flex-wrap gap-3">
        {tabs.map((tab) => (
          <button
            ...
            onClick={() => {
              setActiveTab(tab);
              router.replace(`/organizations/${id}?tab=${encodeURIComponent(tab)}`);
            }}
      ...
      {activeTab === "Overview" ? (
        <div className="grid gap-6 lg:grid-cols-[2fr_1fr]">
          ...
          <Card className="space-y-4">
            ...
            <button
              type="button"
              onClick={() => setShowHealthBreakdown((current) => !current)}
              className="text-xs uppercase tracking-[0.2em] text-primary-light hover:underline"
            >
              {showHealthBreakdown ? "Hide Breakdown" : "View Breakdown"}
            </button>
            {showHealthBreakdown ? (
              <div className="space-y-2 rounded-lg border border-white/10 bg-slate-900/60 p-3 text-sm text-slate-300">
                <div className="flex items-center justify-between">
                  <span>Compliance</span>
                  <span>{healthBreakdown.compliance}%</span>
                </div>
                <div className="flex items-center justify-between">
                  <span>Activity</span>
                  <span>{healthBreakdown.activity}%</span>
                </div>
                <div className="flex items-center justify-between">
                  <span>Usage</span>
                  <span>{healthBreakdown.usage}%</span>
                </div>
                <div className="flex items-center justify-between">
                  <span>Communications</span>
                  <span>{healthBreakdown.communications}%</span>
                </div>
              </div>
            ) : null}
            ...
            <>
              {canManageUsers && !isOrgReadOnly ? (
                ...
                <p className="text-sm text-slate-400">
                  User management actions are disabled while this organisation is suspended.
                </p>
              ) : (
                <p className="text-sm text-slate-400">
                      ...
                      {users.map((userRow) => (
                        <tr key={userRow.id}>
                          ...
                          <td className="px-4 py-3 text-xs text-slate-400 space-y-1">
                            <div>{userRow.isActive ? "Active" : "Inactive"}</div>
                            {isNeverLoggedIn(userRow) ? (
                              <div className="inline-flex rounded-full bg-warning-500/20 px-2 py-0.5 text-warning-500">
                                Never logged in
                              </div>
                            ) : null}
                            {isSetupIncomplete(userRow) ? (
                              <div className="inline-flex rounded-full bg-danger-500/20 px-2 py-0.5 text-danger-500">
                                Setup incomplete
                              </div>
                            ) : null}
                          </td>
                            ...
                            {canManageUsers ? (
                              <div className="flex flex-wrap items-center gap-2">
                                <button
                                  ...
                                  disabled={userActionId === userRow.id || isOrgReadOnly}
                                >
                                  {userRow.isActive ? "Deactivate" : "Reactivate"}
                                </button>
                                {isSetupIncomplete(userRow) || isNeverLoggedIn(userRow) ? (
                                  <button
                                    type="button"
                                    className="rounded-md border border-primary/40 bg-primary/10 px-2 py-1 text-xs text-primary-light"
                                    onClick={() => handleSendResetLink(userRow.id, userRow.email)}
                                    disabled={resetActionId === userRow.id || isOrgReadOnly}
                                  >
                                    {resetActionId === userRow.id ? "Sending..." : "Send Reset Link"}
                                  </button>
                                ) : null}
                              </div>
                            ) : (
